    ...
    async handle(handlerInput) {
        ...
        if(!sessionAttributes['nombre']){
            try{
                ...
                if(!permissions){
                    throw{statusCode: 401, message: 'No permissions avalaible'};
                }
            ...
            speakOutput = requestAttributes.t('WELCOME_MESSAGE', nombre);
        }else{
            nombre = sessionAttributes['nombre']
            speakOutput = requestAttributes.t('WELCOME_MESSAGE_2',nombre);
        }
...
const AdivinanzaIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        let noResp=['idea','sé','se','paso','rindo'];
        let speakOutput;
